    ...
    <div className="max-w-[1200px] mx-auto p-10">
      ...
      <div className="flex justify-between items-start mb-10">
        <div>
          ...
          <p className="text-[var(--vl-text2)] mt-1.5">
            VentureLens AI · {report.name} · {report.industry} · {report.stage}
          </p>
        </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="text-[var(--vl-text)]">{report.name}</span>
          </div>
          <div>
            <span className="text-[var(--vl-text3)]">Indústria: </span>
            <span className="text-[var(--vl-text)]">{report.industry}</span>
          </div>
          <div>
            <span className="text-[var(--vl-text3)]">Estágio: </span>
            <span className="text-[var(--vl-text)]">{report.stage}</span>
          </div>
        </div>
      ...
      <Card>
        ...
        <ol>
          {report.questions.map((question, i) => (
            <li key={i} className="flex gap-3 py-2.5 border-b border-[var(--vl-border)] last:border-0">
              <span className="font-mono text-sm text-[var(--vl-gold2)] min-w-[24px]">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-sm text-[var(--vl-text2)] leading-relaxed">{question}</span>
            </li>
          ))}
        </ol>
      </Card>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wider text-[var(--vl-text3)] border-b border-[var(--vl-border)]">
                ...
                <th className="pb-2">Dica</th>
              </tr>
            </thead>
            <tbody>
              {report.slides.map((slide) => (
                <tr key={slide.n} className="border-b border-[var(--vl-border)] last:border-0">
                  <td className="py-2.5 pr-3 font-mono text-[var(--vl-text3)]">{slide.n}</td>
                  <td className="py-2.5 pr-3 text-[var(--vl-text)]">{slide.t}</td>
                  <td className={`py-2.5 pr-3 font-mono font-bold ${gradeColor(slide.g)}`}>{slide.g}</td>
                  <td className="py-2.5 text-[var(--vl-text2)] leading-relaxed">{slide.tip}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <MetricBox label="Receita" value={report.fin.revenue} />
          <MetricBox label="Burn Rate" value={report.fin.burn} />
          <MetricBox label="LTV:CAC" value={report.fin.ltv_cac} />
          <MetricBox label="Churn" value={report.fin.churn} />
        </div>
      </Card>
            ...
            <tbody>
              {report.comps.map((comp, i) => (
                <tr key={i} className="border-b border-[var(--vl-border)] last:border-0">
                  <td className="py-2.5 pr-3 text-[var(--vl-text)] font-semibold">{comp.name}</td>
                  <td className="py-2.5 pr-3 text-[var(--vl-text2)]">{comp.sim}</td>
                  <td className="py-2.5 text-[var(--vl-text2)]">{comp.out}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card>
        ...
        <div className="mb-5">
          ...
          <div>
            {report.recs.now.map((item, i) => (
              <div key={i} className="flex gap-2 py-2 border-b border-[var(--vl-border)] last:border-0">
